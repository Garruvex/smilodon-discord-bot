...
export const contextScanAddSetting: MutationSettingDefinition = {
  ...
  handle: async (request, deps, previousProfile, input) => {
    ...
    const channel = request.values.getChannel("channel", true);
    const seedDays = request.values.getInteger("seed-days");
    const restart = request.values.getBoolean("restart") ?? false;
...
export const contextDailyAddSetting: MutationSettingDefinition = {
  ...
  handle: (request, deps, _previousProfile, input) => {
    if (!deps.channelSummaryProviderAvailable) {
...
export const contextStatusSetting: ReadOnlySettingDefinition = {
  ...
  run: async (request, deps, profile) => {
    const channelId = request.values.getChannel("channel")?.id ?? null;
