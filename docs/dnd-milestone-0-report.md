...
- **Shops and more consumables**: the inventory holds weapons, armor, shields, and healing potions, and the party has a shared purse of gold found as loot. Spending gold, other potions, and scrolls come with milestone 3 content.
- Death rules are built (a hero who fails three death saves stays dead, their gear goes to the party stash, the player joins a new hero without the old loot, and a lost fight is a story defeat with heroes waking at 1 HP). Items can be given, swapped with consent, and stashed outside combat; in a fight a hero can hand an item to an ally in the same zone (a bonus action) and drink a healing potion (an action, or a bonus action under the BG3-style house rule). Encounters carry loot and gold. The organizer can retry a lost fight from its start with fresh dice.
