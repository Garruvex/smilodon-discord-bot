...
### Discord-owned character library and campaign travel

- A character in **My Characters** belongs to its Discord user ID within this bot instance, across servers. Store a stable library character ID, owner ID, character revision, rules baseline and capability versions, portrait reference, public identity, approved backstory, build, and saved progress snapshots. A campaign hero has its own ID and points to the library character and exact source snapshot. Discord display names and server membership do not establish ownership; recheck the clicking user ID on every private view, import, export, and edit. There is one active hero per player in a campaign, but a player may keep several library characters.
- Joining a lobby offers **Use Saved Character** beside preset and guided creation. The owner picks a character and snapshot, sees a preview of what carries over and what changes, then confirms. Validate class, level, features, spells, items, and house rules against the destination campaign's saved rules and supported content. The organizer may set an entry level and whether imported equipment/progress is allowed. If a build cannot fit, show the exact conflicts and allow an explicit legal rebuild or a new character; never silently discard progress or grant unsupported abilities. Mark Ready only after validation.
- A campaign copy owns its live HP, slots, conditions, inventory, gold, and campaign-specific choices. Other campaigns cannot change those values. At a saved session boundary or campaign end, the owner may **Save Progress** from a stable, committed state into a new immutable library snapshot. Record source campaign, hero, state revision, rules version, level/advancement, legal build choices, and eligible equipment. Do not export unresolved actions, temporary conditions, current HP, spent slots, party stash, or campaign-owned quest items. Destination campaigns initialize transient resources according to their own entry rules.
- Parallel campaigns create separate progression branches from the same snapshot. Saving one branch never overwrites another or advances other active heroes. When joining a later game, the owner chooses the branch/snapshot to use; there is no automatic merge. Keep the provenance so the same reward cannot be claimed twice within one campaign. An organizer correction to the source marks affected snapshots for review before further import rather than silently rewriting an active destination hero.
- Carry a short **player-approved public history** (name, appearance, and selected past deeds) only when the destination organizer accepts character backstories. Campaign secrets, private notes, unrevealed NPC facts, and full transcripts stay in their original campaign. A new setting treats imported history as background unless its organizer deliberately maps it to that world's fiction; the AI DM must not assume old locations or NPCs exist there.
- Deleting or leaving a campaign does not delete the owner's library character. Library deletion is an explicit owner action; retain the minimal historical attribution needed for existing campaign records. A user cannot transfer ownership merely by changing a Discord nickname or handing someone a character link.
...
| Library character/snapshot | Discord owner, stable character ID, immutable source revision, rules/capability versions, approved portable build and history, progression branch and source campaign/hero revision |
...
| 4. Creation, house rules, and art | Adventure upload and the Adventure Author (convert and generate within the ruleset catalog, validated), guided builder, Discord-owned character library with snapshot import/export, house-rule option cards, scene images with budgets | A player reuses a legal character across two campaigns without shared live resources; an incompatible import names its conflicts; two progression branches remain distinct; selected options change actual mechanics; image failures leave text play working |
