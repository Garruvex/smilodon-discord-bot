...
**Items, death, and new heroes.** Gear lives on each hero's sheet plus a shared party stash, and moves only through commands, outside combat: `offerItem` (a gift, or a swap for an item the other hero holds), `respondToOffer` and `cancelOffer` (the receiving hero's owner must accept), `stashItem` and `takeFromStash` (the hero's owner or the organizer). Items are never created by a player's text: an offer can only name items the heroes hold. Loot is authored on encounters and reaches the stash when the party wins. Armor class and attacks are derived from the sheet when a fight starts, so a moved item takes effect from the next fight.

A hero who fails three death saves is fallen for good (`heroStatus.dead`): their gear joins the stash, they leave rounds, fights, and rests, and offers involving them close. A lost fight is a story defeat, not a game over: heroes who are down but not dead wake with 1 HP and the closing narration frames it (captured, driven off, left for dead). `joinHero` gives a player a new hero, either a first one (a new player) or a replacement; the application supplies the sheet from the adventure's presets with starting gear and no loot, and the engine checks the party's level, known content, a fresh ID, and that the player has no living hero. In a fight, hand-overs need the giver's turn, a bonus action, and a receiver standing in the same zone, and the fighters' armor class and attacks are recomputed when the gear changes; healing potions heal their average (no roll) as an action, or a bonus action under the `healing-potion-cost` house rule. The party also holds gold found as loot. A lost fight can be retried: each fight saves a checkpoint of the party (sheets, HP and resources, stash, gold, offers), and `retryEncounter` (organizer only, before anyone acts in the round that follows the defeat) restores it and starts the fight again under the ID `<id>~2`, so every roll is new while the adventure still counts it as one fight.
